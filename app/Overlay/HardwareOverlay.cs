...
namespace GHelper.Overlay
{
    public class HardwareOverlay : OSDNativeForm
    {
        ...
        private const int BaseChartHeight = 44;
        private const int BaseChartGap = 6;
        private const int CornerRadius = 3;
        private const int MarginFromEdge = 10;

        // Colours (background gets 50% alpha painted directly; everything else fully opaque)
        ...
        private void Tick()
        {
            ...
            string cpuTemp = HardwareControl.cpuTemp > 0 ? ": " + Math.Round((decimal)HardwareControl.cpuTemp) + "°C" : "";
            string cpuPower = HardwareControl.cpuPower > 0 ? " " + Math.Round((decimal)HardwareControl.cpuPower, 1) + "W" : "";
            string gpuTemp = HardwareControl.gpuTemp > 0 ? ": " + HardwareControl.gpuTemp + "°C" : "";
            string gpuPower = HardwareControl.gpuPower > 0 ? " " + Math.Round((decimal)HardwareControl.gpuPower, 1) + "W" : "";
