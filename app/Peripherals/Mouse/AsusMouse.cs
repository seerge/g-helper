...
// modifiers
        ...
        // Old protocol (Chakram X etc.) — base common buttons only
        public static readonly IReadOnlyList<(string GroupLabel, IReadOnlyList<(ushort Code, string Name)> Items)>
        OldProtocolBindingGroups = new List<(string, IReadOnlyList<(ushort, string)>)>
        {
            ("Mouse", new List<(ushort, string)>
            {
                (0x01F0, "Mouse Left"   ),
        ...
        public static readonly Dictionary<ushort, string> NewProtocolBindingCodes =
            NewProtocolBindingGroups.SelectMany(g => g.Items)
                .ToDictionary(e => e.Code, e => e.Name);

        public static readonly Dictionary<ushort, string> OldProtocolBindingCodes =
            OldProtocolBindingGroups.SelectMany(g => g.Items)
                .ToDictionary(e => e.Code, e => e.Name);

        // Per-instance resolved groups and codes based on protocol
        public virtual IReadOnlyList<(string GroupLabel, IReadOnlyList<(ushort Code, string Name)> Items)>
            InstanceBindingGroups => BindingProtocol == ButtonBindingProtocol.Old
                ? OldProtocolBindingGroups : NewProtocolBindingGroups;

        public virtual string LabelForActionCode(ushort code)
        {
            var dict = BindingProtocol == ButtonBindingProtocol.Old
                ? OldProtocolBindingCodes : NewProtocolBindingCodes;
            return dict.TryGetValue(code, out var n) ? n : $"Unknown (0x{code:X4})";
        }
        ...
        public enum ButtonBindingProtocol { None, New, Old }
