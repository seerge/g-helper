    ...
    public class GPUModeControl
    {
        ...
        public void InitGPUMode()
        {
            ...
            if (eco == 1 && HardwareControl.GpuControl?.IsValid == true)
            {
                ...
                if (AppConfig.IsEcoBootFix()) Program.acpi.DeviceSet(AsusACPI.GPUEco, eco, "GPUEco Force Fix");
            }
        ...
        public bool SetGPUEco(int eco)
        {
            ...
            if (eco == 1 && NvOptimusMux.TryGetState(out _, out var mux) && mux == NvOptimusMux.OptimusMuxState.DGpu)
            {
                NvOptimusMux.OpenTray();
                MessageBox.Show(
                    settings,
