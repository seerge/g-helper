...
$title = "GPU Mode"
$title_eco = "Mode: iGPU only"
$title_standard = "Mode: iGPU and dGPU"
$title_ultimate = "Mode: dGPU exclusive"
...
$Main_Tool_Icon.Text = "G14 GPU Mode"
$Main_Tool_Icon.Icon = $icon_standard
$Main_Tool_Icon.Visible = $true

$Menu_Title = New-Object System.Windows.Forms.MenuItem
$Menu_Title.Enabled = $false
$Menu_Title.Text = $title

$Menu_Eco = New-Object System.Windows.Forms.MenuItem
$Menu_Eco.Text = "Eco"

$Menu_Standard = New-Object System.Windows.Forms.MenuItem
$Menu_Standard.Text = "Standard"
 
$Menu_Ultimate = New-Object System.Windows.Forms.MenuItem
...
$Menu_RR120 = New-Object System.Windows.Forms.MenuItem
$Menu_RR120.Text = "120Hz"

$Menu_OD = New-Object System.Windows.Forms.MenuItem
$Menu_OD.Text = "Panel Overdrive"
...
$global:eco_mode = (Invoke-CimMethod $asushw -MethodName DSTS -Arguments @{Device_ID=$device_eco} | Select-Object -ExpandProperty device_status) - 65536;
$global:ultimate_mode = 0;

if ($mux_mode -eq 0) {
    $global:ultimate_mode = 1;
} elseif ($mux_mode -eq 1) {
    $global:ultimate_mode = 0;
} else {
...
$Menu_Eco.add_Click({
    ...
    if ($global:ultimate_mode -eq 1) {
        $msgBox =  [System.Windows.MessageBox]::Show('Eco mode requires restart','Reboot now?','OKCancel')
        if  ($msgBox -eq 'OK') {
            $global:ultimate_mode = 0
            CheckMode
...
$Menu_Standard.add_Click({
    ...
    if ($global:ultimate_mode -eq 1) {
        $msgBox =  [System.Windows.MessageBox]::Show('Standard mode requires restart','Reboot now?','OKCancel')
        if  ($msgBox -eq 'OK') {
            $global:ultimate_mode = 0
            CheckMode
            Invoke-CimMethod $asushw -MethodName DEVS -Arguments @{Device_ID=$device_mux ; Control_status=1 }
...
$Menu_Ultimate.add_Click({
    ...
    if ($global:ultimate_mode -eq 1) {return}
...
$Menu_OD.add_Click({
    if ($Menu_OD.Checked) {
        Invoke-CimMethod $asushw -MethodName DEVS -Arguments @{Device_ID=$device_overdrive ; Control_status=0 }
    } else {
        Invoke-CimMethod $asushw -MethodName DEVS -Arguments @{Device_ID=$device_overdrive ; Control_status=1 }
    }
    CheckScreen
})
